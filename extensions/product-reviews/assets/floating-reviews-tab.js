(function () {
  ...
  function initAllFloatingReviewTabs(scope = document) {
    const roots = Array.from(scope.querySelectorAll(".frt-root"));
    if (!roots.length) return;

    roots.forEach((root) => {
      if (root.dataset.initialized === "true") return;
      root.dataset.initialized = "true";
      initFloatingReviewsTab(root);
    });
  }

  document.addEventListener("DOMContentLoaded", () => {
    initAllFloatingReviewTabs(document);
  });

  document.addEventListener("shopify:section:load", (event) => {
    initAllFloatingReviewTabs(event.target || document);
  });
  ...
  function initFloatingReviewsTab(root) {
    const shop = root.dataset.shop || "";
    const endpoint = root.dataset.endpoint || "";
    const reviewType = (root.dataset.reviewType || "store").trim().toLowerCase();
    const maxReviews = Math.max(1, Number(root.dataset.maxReviews || 12));
    const hideTriggerWhenOpen =
      String(root.dataset.hideTriggerWhenOpen || "true").toLowerCase() === "true";
    ...
    if (hideTriggerWhenOpen && trigger) {
    ...
    function renderStarsText(rating) {
      ...
      const full = "★".repeat(Math.round(safeRating));
    ...
    function closeOtherTabs() {
      ...
      openRoots.forEach((otherRoot) => {
        ...
        const closeEvent = new CustomEvent("frt:close-self");
        otherRoot.dispatchEvent(closeEvent);
      });
    }
    ...
    trigger && trigger.addEventListener("click", togglePanel);
