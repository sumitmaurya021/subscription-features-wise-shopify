(function () {
  const PRODUCT_BADGE_ROOT_SELECTOR = ".pr-star-rating-badge-root[data-badge-type='product']";
  const COLLECTION_BADGE_ROOT_SELECTOR = ".pr-collection-star-rating-badges-root[data-badge-type='collection']";
  ...
  function boolFromData(value, fallback = false) {
  ...
  function buildBadgeMarkup({
    ...
    return `
      ...
      >
        ...
        ${showRatingNumber ? `<span class="pr-star-badge__rating">${escapeHtml(ratingText)}</span>` : ""}
        ${showReviewCount ? `<span class="pr-star-badge__count">${escapeHtml(reviewText)}</span>` : ""}
      </${tagName}>
    `;
  }
  ...
  function findProductCard(linkEl) {
    ...
    return linkEl.closest(PRODUCT_CARD_SELECTORS) || linkEl.closest("li") || linkEl.parentElement;
  }
  ...
  function findExistingProductId(card, linkEl) {
    ...
    for (const element of candidates) {
      const productId = element?.dataset?.productId || element?.getAttribute?.("data-product-id");
  ...
  async function initCollectionBadges(root) {
    ...
    const uniqueProductIds = [...new Set(validItems.map((item) => safeText(item.productId)))];
  ...
  async function bootStarBadges() {
    const productRoots = Array.from(document.querySelectorAll(PRODUCT_BADGE_ROOT_SELECTOR));
    await Promise.all(productRoots.map(initProductBadge));

    const collectionRoots = Array.from(document.querySelectorAll(COLLECTION_BADGE_ROOT_SELECTOR));
    for (const collectionRoot of collectionRoots) {
      await initCollectionBadges(collectionRoot);
    }
  }

  document.addEventListener("DOMContentLoaded", bootStarBadges);
  document.addEventListener("shopify:section:load", bootStarBadges);
})();
